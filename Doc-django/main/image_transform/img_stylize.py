...
def stylize_images(image, filter, alpha_value=128):
    ...
    os.remove(new_image_jpg)
    os.remove(new_filter_jpg)
    ...
    os.remove(new_image_png)
    os.remove(new_filter_png)
...
def convert_jpg_to_png(photo_jpg):
    """
    Конвертирует изображение из jpg и jpeg в png
    Возвращаем новый путь к картинке.
    """
    img = Image.open(photo_jpg)
    photo_png = re.sub(r'.jpg|.jpeg$', '.png', photo_jpg)
    img.save(photo_png)
    return photo_png
